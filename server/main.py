...
async def lifespan(app: FastAPI):
    print("=== Iniciando Servidor TTS (Crystal Audio Edition) ===")
...
def align_and_cast(session, input_name, data):
    """Alineación de tensores profesional."""
    for inp in session.get_inputs():
        if input_name in inp.name:
            ...
            if len(e_s) == len(data.shape):
                for i in range(len(e_s)):
                    if isinstance(e_s[i], int) and e_s[i] > 0 and e_s[i] != data.shape[i]:
                        for j in range(len(data.shape)):
                            if data.shape[j] == e_s[i]:
                                axes = list(range(len(data.shape))); axes[i], axes[j] = axes[j], axes[i]
...
async def synthesize(data: str = Form(...)):
    ...
    # --- ETAPA B: TRANSFORMER (Difusión con refinamiento de tiempo) ---
    steps = 30
    dt = 1.0 / steps
    for step in range(steps):
        # Probamos tiempo normalizado para máxima estabilidad
        t = float(step) / steps
        trans_in = {}
        for inp in models["sess_trans"].get_inputs():
            if 'noise' in inp.name: trans_in[inp.name] = noise
            elif 'time_step' in inp.name:
                val = np.array([t], dtype=np.float32 if 'float' in inp.type else np.int64)
    ...
    # --- PROCESAMIENTO DE AUDIO FINAL (SOLUCIÓN DEFINITIVA RUIDO) ---
    audio_flat = audio_out.flatten().astype(np.float32)
    
    # 1. Normalización de Amplitud (Evitar clipping)
    audio_flat = audio_flat - np.mean(audio_flat)
    max_amp = np.max(np.abs(audio_flat))
    if max_amp > 0:
        audio_flat = audio_flat / max_amp * 0.8
    
    # 2. Forzar Little Endian y PCM 16-bit (Clave para eliminar ruido EDM)
    # Convertimos a entero de 16 bits usando el orden de bytes nativo de Android
    pcm_ints = (audio_flat * 32767).astype('<i2')
